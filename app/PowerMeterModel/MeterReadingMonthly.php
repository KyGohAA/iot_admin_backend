...
class MeterReadingMonthly extends ExtendModel
{
    protected $table = 'meter_reading_monthlys';
    ...
    */
    ...
    {
        $model = MeterReadingMonthly::where('meter_register_id','=',$meter_register_id)
                                    ->leftJoin('meter_registers','meter_registers.id','=','meter_reading_monthlys.meter_register_id')
                                    ->whereBetween('current_date', [date('Y-m').'-01', date('Y-m-d')])
                                    ->groupBy('meter_register_id')
                                    ->selectRaw('sum(meter_reading_monthlys.current_usage) as current_usage, meter_registers.leaf_room_id as room_id,meter_registers.reading_status as reading_status , meter_registers.id as meter_register_id')
                                    ->get();
    ...
    public static function get_monthly_meter_reading_from_now($meter_register_id)
    {
        $model = MeterReadingMonthly::where('meter_register_id','=',$meter_register_id)
                                    ->whereBetween('current_date', [date('Y-m').'-01', date('Y-m-d')])
                                    ->groupBy('meter_register_id')
    ...
    public static function get_selected_monthly_meter_reading_from_now($meter_register_ids)
    {
        $model = MeterReadingMonthly::whereIn('meter_register_id',$meter_register_ids)
                                    ->whereBetween('current_date', [date('Y-m').'-01', date('Y-m-d')])
                                    ->groupBy('meter_register_id')
                                    ->selectRaw('sum(current_usage) as current_usage, meter_register_id as meter_register_id')
                                    ->first();
    ...
    {
        $listing = DB::select('SELECT `current_date` as month_year, SUM(`current_usage`) as total_amount FROM `meter_reading_monthlys` WHERE `current_date` BETWEEN ? AND ? GROUP BY `current_date`', [$date_range('date_started'), $date_range('date_ended')]);
            $current_usage = '';
            if (count($listing)) {
                $current_usage .= '[';
    ...
    {
        $listing = DB::select('SELECT `current_date` as date_time, SUM(`current_usage`) as total_amount FROM `meter_reading_monthlys` WHERE `current_date` BETWEEN ? AND ? GROUP BY YEAR(`current_date`), MONTH(`current_date`)', [$date_range['date_started'], $date_range['date_ended']]);
            $current_usage = '';
            if (count($listing)) {
                $current_usage .= '[';
                foreach ($listing as $row) {
            ...
            foreach ($meter_register_listing as $meter_row) {
                ...
                foreach ($today_reading_listing as $reading_row) {
                    if($reading_row['meter_register_id'] == $meter_row['id']){
                        $monthly_meter_reading_listing = MeterReading::get_monthly_meter_reading_from_now($meter_row['id'],$date_range);
                        $reading_row['current_usage'] = $monthly_meter_reading_listing[0]['current_usage'];
                ...
                if(!$is_record_update){
                    static::save_monthly_meter_reading_by_meter_register_id($meter_row['id'] , $date_range);
                }
    ...
    public static function save_monthly_meter_reading_by_meter_register_id($id, $number_of_months_from_now){
       
        $number_of_months_from_now = 24;
        $monthly_meter_reading_listing = MeterReading::get_monthly_meter_reading_from_now($id,$number_of_months_from_now);
        
       /* if(is_object($monthly_meter_reading_listing) == true){
            dd("objesc");*/
          if(count($monthly_meter_reading_listing) > 0){
                foreach ($monthly_meter_reading_listing as $row) {         
                   // print_r(json_encode($row)); 
                    echo $row->month_year.'='.$row->total_amount.'<br>';
                   ...
                   /* if(isset($model['id']) == true){
                    ...
                    $model = new MeterReadingMonthly();
                   // dd($model);
                    $model->save_form_by_meter_reading_monthly_summary($row);
    ...
    public static function save_monthly_meter_reading_by_leaf_group_id($leaf_group_id,$date_range=null){

        $meter_register_listing = MeterRegister::get_active_meter_register_by_leaf_group_id(Setting::get_leaf_group_id($leaf_group_id));

        if(count($meter_register_listing) > 0){
            foreach ($meter_register_listing as $row) {
                static::save_monthly_meter_reading_by_meter_register_id($row['id'],$date_range);
            }
        }
    ...
    public static function save_all_monthly_meter_reading_by_leaf_group_id($leaf_group_id,$date_range=null){

        static::save_all_monthly_meter_reading_by_meter_register_id($leaf_group_id ,24);
    }

    public static function save_all_monthly_meter_reading_by_meter_register_id($leaf_group_id, $number_of_months_from_now){
       
        $meter_register_listing = MeterRegister::get_active_meter_register_by_leaf_group_id($leaf_group_id);
        $meter_register_ids = array_column($meter_register_listing->toArray(), 'id');
        $number_of_months_from_now = 24;
        $monthly_meter_reading_listing = MeterReading::get_selected_monthly_meter_reading_from_now($meter_register_ids,$number_of_months_from_now);
      //  dd( $monthly_meter_reading_listing[0]);
       /* if(is_object($monthly_meter_reading_listing) == true){
            dd("objesc");*/
          if(count($monthly_meter_reading_listing) > 0){
                foreach ($monthly_meter_reading_listing as $row) {         
    ...
    */
    ...
    //cont meter_reading_to_model_key_mapping = ['total']
    public function save_form_by_meter_reading_monthly_summary($row)
    {   echo json_encode($row).'<br>';
        DB::beginTransaction();
        try {
                foreach ($row as $key => $value) {
                //foreach ($row->getAttributes() as $key => $value) {
                    if ($key != 'date_time' && $key != 'total_amount' && $key != 'month_year') {
                        $this->$key = (string) $value;
                    }else if($key == 'total_amount'){
                        $this->current_usage = $value;

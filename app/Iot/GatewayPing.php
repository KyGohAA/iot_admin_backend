...
class GatewayPing extends ExtendModel
{   
    protected $table = 'gateway_ping';
    ...
    */
    ...
    public function validate_form($input)
    {
        $rules = [
                    'state_id'  =>  'required',
                    'code'      =>  'required|unique:gateway_ping,code,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    'name'      =>  'required|unique:gateway_ping,name,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    ];

        if ($this->id) {
            $rules['code'] = 'required|unique:gateway_ping,code,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
            $rules['name'] = 'required|unique:gateway_ping,name,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
        }

...
class NetworkServer extends ExtendModel
{
    protected $table = 'network_server';
    public $timestamps = true;
     protected $listing_except_columns = ['created_at', 'updated_at','ca_cert','tls_cert','tls_key','routing_profile_ca_cert','routing_profile_tls_cert','routing_profile_tls_key'];
    ...
    */
    ...
    public function validate_form($input)
    {
        $rules = [
                    'state_id'  =>  'required',
                    'code'      =>  'required|unique:network_server,code,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    'name'      =>  'required|unique:network_server,name,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    ];

        if ($this->id) {
            $rules['code'] = 'required|unique:network_server,code,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
            $rules['name'] = 'required|unique:network_server,name,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
        }

...
class PaymentReceivedPdf extends Fpdf
{
	...
	public function getHeaderFromCompanyModel(){
		...
		$header_data = [
                    'status_code'   =>  1,
                    'address'    =>  isset($model) ? $model->get_address() : "",
                    'company' => $model,
                    ];
    ...
    public function content($model,$membership_detail)
    {
		...
		$this->setX(55);
		$this->Cell(0,3,$this->setting->convert_encoding('GST Registration No:').$this->header_data['company']['gst_registration_no'],0,1,'L');
	    $this->SetFont('Courier', '', 10);
	    $this->setX(90);
	    $this->Ln(1);
        ...
        $this->Cell(30,3,$this->setting->convert_encoding(Language::trans('Receipt')),0,1,'L');
		...
		$this->Cell(25,5,$this->setting->convert_encoding(Language::trans('Amount (RM)')),0,1,'C');
		...
		foreach ($model->items as $row) {
			...
			$this->Cell(30,5,($index+1).'.',0,0,'L');
			$this->Cell(130,5,$this->setting->convert_encoding($row->description),0,0,'L');
			$this->Cell(25,5,$this->setting->convert_encoding($row->getDouble($row->received_amount)),0,1,'R');		
/*			$this->Line(180, $this->GetY(), 200, $this->getY());
			$this->Line(180, $this->GetY(), 200, $this->getY());*/
			$total+=$row->received_amount;
		}
	/*	$this->Line(5, $this->GetY(), 210, $this->GetY());
		$this->Ln(5);
		$this->Cell(150,5,$this->setting->convert_encoding(Language::trans('Total')).' : ',0,0,'R');
		$this->Cell(25,5,$this->setting->convert_encoding($this->setting->getDouble($model['payment_amount'])),0,1,'R');
		$this->Ln(2);*/
		...
		$this->Cell(215,5,$this->setting->convert_encoding(Language::trans('This Official Receipt is valid subject to the cheque being honoured. Thanks you for your payment.')).' : ',0,0,'L');
		$this->Ln(2);*/
		// footer of the table
...
/*        $this->SetX(170);
        $this->SetFont('arial','',10);
        $this->Cell(10,4,'SST @ 10.00% : ',0,0,'R');
        ...
        $this->Cell(150,4, $this->setting->convert_encoding('This Official Receipt is valid subject to the cheque being honoured. Thanks for your payment.'),0,1,'L');

        $this->SetY(-26);
        $this->line(5,$this->getY(),50,$this->getY());
        $this->SetY(-24);
        $this->Cell(30,4,'Received By',0,0,'L');
        $this->SetY(-20);

...
class CustomerGroup extends ExtendModel
{
    ...
    */
    ...
    public static function combobox()
    {
        return static::ofAvailable('status',true)
                                ->pluck('name','id')
                                ->prepend(Language::trans('Please select customer group...'), '');
    }
    ...
    */
    ...
    {
        $rules = [
                    'code'      =>  'required|unique:customer_groups,code,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    'name'      =>  'required|unique:customer_groups,name,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    ];

        if ($this->id) {
            $rules['code'] = 'required|unique:customer_groups,code,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
            $rules['name'] = 'required|unique:customer_groups,name,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
        }

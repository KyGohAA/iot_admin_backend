...
class PaymentTerm extends ExtendModel
{
    ...
    */
    ...
    public static function combobox()
    {
        return static::ofAvailable('status',true)
                                ->pluck('code','id')
                                ->prepend(Language::trans('Please select payment terms...'), '');
    }

    public static function get_common_combobox()
    {
    ...
    */
    ...
    {
        $rules = [
                    'code'      =>  'required|unique:payment_terms,code,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    'days'      =>  'required|unique:payment_terms,days,NULL,id,leaf_group_id,'.Company::get_group_id(),
                    ];

        if ($this->id) {
            $rules['code'] = 'required|unique:payment_terms,code,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
            $rules['days'] = 'required|unique:payment_terms,days,'.$this->id.',id,leaf_group_id,'.Company::get_group_id();
        }
    ...
    public function save_form($input)
    {
    	...
    	try {
            if ($input['is_default']) {
                DB::table('payment_terms')->update(['is_default'=>false]);
            }

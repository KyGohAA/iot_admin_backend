...
<section class="hk-sec-wrapper">
 ...
 <h5 class="hk-sec-title">{{App\Language::trans('User')}}</h5><hr>
 <div class="row">
						<div class="col-md-12">
						        <div class="form-group {!!$errors->first('code') ? 'has-error' : ''!!}">
						          <label for="code" class="control-label col-sm-2">{{App\Language::trans('User')}}</label>
							          <div class="col-sm-12">
							           {!!Form::select("user_assign[user_list][]", App\User::user_assign_combobox($model['id']), isset($user_assign['user_list']) ? ( strlen($user_assign['user_list']) >  1 ? json_decode($user_assign['user_list'] ,true):null ) : null , array(  "style"=>"width: 100%;","class"=>"form-control 3col active","id"=>"user_assign[user_list][]","multiple"=>"multiple"))!!}
							          </div>
						        </div>
					    </div>
					</div>

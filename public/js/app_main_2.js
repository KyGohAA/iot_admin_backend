$(document).ready(function() {
	...
	/*  */
        ...
        if(url.includes('utility_charges/dashboard'))
        {
             init_dougnut_chart();
        ...
        }else if(url.includes('payment-received/new'))
        {

            var product_listing = document.getElementById("product_listing");
            product_listing.addEventListener("click", function(me){
                 id_type = 'leaf_product_id';
                 $.get(productInfoUrl, {product_id:document.getElementById("product_listing").value , leaf_group_id : $('#leaf_group_id').val() ,  id_type:id_type}, function(fdata)
                 {     
                    for (var key in fdata.data) {
                        if(key == "amount") {
                           $('#top_up_amount_txt').val(fdata.data[key]);
                           $(amount).html(fdata.data[key]);
                        }
                    }
...
function init_dougnut_chart($id) {

    $.get(usageLineChartDataUrl, {
        userId: $id
    }, function(data) {
		//$('#test').val($('#statistic').val());
        var donutData = eval("[" + data.data + "]");

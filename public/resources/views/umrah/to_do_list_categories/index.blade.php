					...
					@foreach($model as $index => $row)
						<tr>
							...
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" href="{{action('ToDoListCategoriesController@getEdit', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 
								<a href="{{action('ToDoListCategoriesController@getView', [$row->id])}}">{{App\Language::trans('View')}}</a> | 
								<a onclick="return confirm(confirmMsg)" href="{{action('ToDoListCategoriesController@getDelete', [$row->id])}}">{{App\Language::trans('Del')}}</a>
							</td>
						</tr>
					@endforeach
				</tbody>

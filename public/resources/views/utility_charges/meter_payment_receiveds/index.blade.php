                    ...
                    @foreach($model as $index => $row)
                        <tr>
                            ...
                            <td class="text-center">
                                <a onclick="return confirm(confirmMsg)" href="{{action('UMeterPaymentReceivedsController@getEdit', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 
                                <a href="{{action('UMeterPaymentReceivedsController@getView', [$row->id])}}">{{App\Language::trans('View')}}</a> | 
                                <a target="_blank" href="{{action('UMeterPaymentReceivedsController@getPrint', [$row->id])}}">{{App\Language::trans('Print')}}</a> | 
                                <a onclick="return confirm(confirmMsg)" href="{{action('UMeterPaymentReceivedsController@getDelete', [$row->id])}}">{{App\Language::trans('Del')}}</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>

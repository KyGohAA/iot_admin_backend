				...
				<thead>
					<tr>
						@foreach($cols as $col)
							...
							@elseif(str_contains($col, 'is_'))
								<th class="text-center">{{App\Language::trans(ucwords(str_replace('is_', '', $col)))}}</th>
							@else
						...
						<tr>
							...
							@foreach($row->toArray() as $key => $value)
								...
								@elseif(str_contains($key, 'is_'))
									<td class="text-center">{{$value ? App\Language::trans('Yes'):App\Language::trans('No')}}</td>
								@elseif($key != 'id')
									<td class="text-center">{{$value}}</td>
								@endif
							@endforeach
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" href="{{action('UMeterClassController@getEdit', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 
								<a href="{{action('UMeterClassController@getView', [$row->id])}}">{{App\Language::trans('View')}}</a> | 
								<a onclick="return confirm(confirmMsg)" href="{{action('UMeterClassController@getDelete', [$row->id])}}">{{App\Language::trans('Del')}}</a>
							</td>
						</tr>
					@endforeach
				</tbody>

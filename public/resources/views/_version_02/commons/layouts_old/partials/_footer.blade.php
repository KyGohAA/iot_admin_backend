...
<script>
  ...
  //power meter
  var power_meter_house_listing_url = "{{action('UMeterRegistersController@getStatusDetailData')}}";
  @yield('script')
  
 /* setInterval(function(){
    $.get(checkCustomerLatest, function(data){
      if (data.status_code) {
        $.toast({
            heading: 'Information',
            text: data.user_fullname+"{{App\Language::trans(' just in front of you.')}} <a class='btn btn-info' target='_blank' href='{{action('CustomersController@getDetails')}}?user_email="+data.user_email+"'>{{App\Language::trans('More Detail')}}</a>",

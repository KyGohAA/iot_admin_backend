					...
					@foreach($model as $index => $row)
						<tr>
							...
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" class="loading-label" href="{{action('UMeterInvoiceController@getEdit', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 
								<a href="{{action('UMeterInvoiceController@getView', [$row->id])}}">{{App\Language::trans('View')}}</a>
							</td>
						</tr>
					@endforeach
				</tbody>

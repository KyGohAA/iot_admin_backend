		...
		<div class="row">
			<div class="col-md-6">
				<div class="form-group{{ $errors->has('leaf_house_id') ? ' has-error' : '' }}">
					...
					<div class="col-md-8">
						<p class="form-control-static">{{$model->convert_house_no($model->display_relationed('meter_register', 'leaf_room_id'), $rooms)}}</p>
					</div>
				</div>
			</div>
			<div class="col-md-6">
		...
		<div class="row">
			<div class="col-md-6">
				<div class="form-group{{ $errors->has('leaf_room_id') ? ' has-error' : '' }}">
					...
					<div class="col-md-8">
						<p class="form-control-static">{{$model->convert_room_no($model->display_relationed('meter_register', 'leaf_room_id'), $rooms)}}</p>
					</div>
				</div>
			</div>
			<div class="col-md-6">
				<div class="form-group{{ $errors->has('over_due_amount') ? ' has-error' : '' }}">
					...
					<div class="col-md-8">
						<p class="form-control-static">{{$model->over_due_amount}}</p>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-6">
				<div class="form-group{{ $errors->has('current_meter_reading') ? ' has-error' : '' }}">
					...
					<div class="col-md-8">
						<p class="form-control-static">{{$model->current_meter_reading}}</p>
					</div>
				</div>
			</div>
			<div class="col-md-6">
				<div class="form-group{{ $errors->has('last_meter_reading') ? ' has-error' : '' }}">
					...
					<div class="col-md-8">
						<p class="form-control-static">{{$model->last_meter_reading}}</p>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="table-responsive">
					...
					<table id="detail_prices" class="table table-bordered table-hover">
						...
						<tbody>
							...
							<tr class="gst">
								<td class="col-md-6 text-center">6% GST {{-- (6% X RM<span class="total_gst_amount">{{$model->getDouble(($model->gst_amount/6)*100)}}</span>) --}}<span class="pull-right col-md-1">RM</span></td>
								<td class="col-md-2 text-center"></td>
								<td class="col-md-2 text-center"></td>

					...
					@foreach($model as $index => $row)
						<tr>
							...
							@foreach($row->toArray() as $key => $value)
								@if($key == 'photo')
									<td class="col-md-1"><img class="img-responsive" src="{{$row->profile_jpg()}}"></td>
								@elseif($key == 'status')
									<td class="text-center">{{$row->display_status_string($key)}}</td>
								@elseif($key != 'user_id' && str_contains($key, '_id'))
							...
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" class="loading-label" href="{{action('UsersController@getEdit', [$row->user_id])}}">{{App\Language::trans('Edit')}}</a>
							</td>
						</tr>
					@endforeach
				</tbody>

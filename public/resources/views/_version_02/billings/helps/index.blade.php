	...
	<div class="row">
        <div class="col-md-6 col-sm-6 col-xs-12">
          <div class="info-box">
            ...
            <div class="info-box-content">
              <span class="info-box-text">{{App\Language::trans('Contact Us')}}</span>
              <span class="info-box-number">{{App\Language::trans('Disabled')}}</span>
              <span>
	              <a href="{{action('HelpsController@getNew')}}" class="btn btn-small btn-info">
					<i class="fa fa-file"></i> {{App\Language::trans('Edit')}}
				  </a>
			  </span>
            </div>
            <!-- /.info-box-content -->
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
          <div class="info-box">
            ...
            <div class="info-box-content">
              <span class="info-box-text">{{App\Language::trans('Help')}}</span>
              <span class="info-box-number">{{App\Language::trans('Disabled')}}</span>
               <span>
	              <a href="{{action('HelpsController@getNew')}}" class="btn btn-small btn-info">
					<i class="fa fa-file"></i> {{App\Language::trans('Edit')}}
					...
					@foreach($model as $index => $row)
						<tr>
							...
							@foreach($row->toArray() as $key => $value)
								...
								@elseif($key == 'content')
									<td class="text-center">{!!html_entity_decode($value)!!}</td>
								@elseif($key != 'id')
									<td class="text-center">{{$value}}</td>
								@endif
							@endforeach
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" class="loading-label" href="{{action('MobileLayoutsController@getView', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 

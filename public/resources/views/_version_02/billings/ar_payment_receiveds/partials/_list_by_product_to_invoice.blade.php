				...
				<div class="table-responsive">
					<table id="product_table" class="table table-bordered table-hover" style="min-width: 1000px;">
						...
						<tbody>
							@foreach($model->items as $row)
								<tr>
									<td>{{$i}}</td>
									<td>
										{!!Form::select('products['.$i.'][product_id]', App\Product::combobox(), $row->product_id, ['class'=>'form-control','onchange'=>'init_product_info(this, "sales","arpa")','required'])!!}
									</td>
									<td>
										{!!Form::textarea('products['.$i.'][description]', $row->product_description, ['rows'=>6,'class'=>'form-control'])!!}
									</td>
									<td>
										{!!Form::number('products['.$i.'][quantity]', $row->quantity, ['class'=>'form-control margin-bottom-15','step'=>'1','onchange'=>'init_calculate_row(this, false,"arpa")'])!!}
										{!!Form::text('products['.$i.'][uom]', $row->uom, ['class'=>'form-control margin-bottom-15'])!!}
										{!!Form::number('products['.$i.'][unit_price]', $row->unit_price, ['class'=>'form-control','step'=>'0.01','onchange'=>'init_calculate_row(this, true,"arpa")'])!!}
									</td>
									<td>
										{!!Form::select('products['.$i.'][tax_id]', App\Tax::combobox(App\Tax::sale_tag), $row->tax_id, ['class'=>'form-control margin-bottom-15'])!!}
										{!!Form::number('products['.$i.'][tax_percent]', $row->tax_percent)!!}
										{!!Form::number('products['.$i.'][tax_txt]', $row->tax_txt, ['class'=>'form-control margin-top-15','step'=>'0.01'])!!}
									</td>
									<td>
										{!!Form::text('products['.$i.'][amount]', $row->amount, ['class'=>'form-control margin-bottom-15 text-right','step'=>'0.01','readonly'])!!}
									</td>
							...
							<tr>
								<td>{{$i}}</td>
								<td>
									{!!Form::select('products['.$i.'][product_id]', App\Product::combobox(), null, ['class'=>'form-control','onchange'=>'init_product_info(this, "sales","arpa")','required'])!!}
								</td>
								<td>
									{!!Form::textarea('products['.$i.'][description]', null, ['rows'=>6,'class'=>'form-control'])!!}
								</td>
								<td>
									{!!Form::number('products['.$i.'][quantity]', null, ['class'=>'form-control margin-bottom-15','step'=>'1','onchange'=>'init_calculate_row(this, false,"arpa")'])!!}
									{!!Form::text('products['.$i.'][uom]', null, ['class'=>'form-control margin-bottom-15'])!!}
									{!!Form::number('products['.$i.'][unit_price]', null, ['class'=>'form-control','step'=>'0.01','onchange'=>'init_calculate_row(this, true,"arpa")'])!!}
								</td>
								<td>
									...
									{!!Form::number('products['.$i.'][tax_txt]', null, ['class'=>'form-control margin-top-15','step'=>'0.01'])!!}
								</td>
								<td>
									{!!Form::number('products['.$i.'][amount]', null, ['class'=>'form-control margin-bottom-15 text-right','step'=>'0.01','readonly'])!!}
								</td>
								<td class="text-center">
									<a class="btn btn-default" onclick="remove_row(this)" href="javascript:void(0)"><i class="fa fa-trash fa-fw"></i></a>
								</td>
						...
						<tfoot>
							...
							<tr>
								<td class="text-right" colspan="5">{{App\Language::trans('Tax')}} :</td>

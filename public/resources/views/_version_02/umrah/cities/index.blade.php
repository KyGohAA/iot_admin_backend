					...
					@foreach($model as $index => $row)
						<tr>
							...
							<td class="text-center">
								<a onclick="return confirm(confirmMsg)" class="loading-label" href="{{action('CitiesController@getEdit', [$row->id])}}">{{App\Language::trans('Edit')}}</a> | 
								<a class="loading-label" href="{{action('CitiesController@getView', [$row->id])}}">{{App\Language::trans('View')}}</a> | 
								<a onclick="return confirm(confirmMsg)" class="loading-label" href="{{action('CitiesController@getDelete', [$row->id])}}">{{App\Language::trans('Del')}}</a>
							</td>
						</tr>
					@endforeach
				</tbody>
